...
//			return Param.getParamNames ( keys );
		...
		bool searchParam (string key, string result_out)
		{
			...
			string remapped = key;
			if ( unresolved_remappings.Contains ( key ) )
				remapped = (string) unresolved_remappings [ key ];

			return Param.search ( resolveName ( "" ), remapped, ref result_out );
		}

		public void param<T> (string param_name, ref T param_val, T default_val)
		{
